...
class AbstractTableTest {
    ...
    void testNotEmptyRangeConstructor() {
        ...
        Map<TableColumn, Integer> headerDescriptionMap = Map.of(Columns.FIRST.getColumn(), Columns.FIRST.getColumn().getColumnIndex(headerRows), Columns.SECOND.getColumn(), Columns.SECOND.getColumn().getColumnIndex(headerRows));
        assertEquals(report, table.getReportPage());
    ...
    void testEmptyRangeConstructor2() {
        ...
        TableCellRange range = TableCellRange.of(1, 5, table.getTableRange().getFirstColumn(), table.getTableRange().getLastColumn());
        assertEquals(report, table.getReportPage());
        assertEquals(range, table.getTableRange());
        assertEquals(originalTable.getHeaderDescription(), table.getHeaderDescription());
        assertFalse(table.isEmpty());
    ...
    void testEqualsAndHashCode() {
        // no subclass for test
        EqualsVerifier.forClass(AbstractTable.class).suppress(STRICT_INHERITANCE).verify();
    }
    ...
    class TableTestImpl extends AbstractTable<EmptyTableRow> {
        protected <T extends Enum<T> & TableHeaderColumn> TableTestImpl(AbstractReportPage<EmptyTableRow> reportPage, String tableName, TableCellRange tableRange, Class<T> headerDescription, int headersRowCount) {
    ...
    enum Columns implements TableHeaderColumn {
        FIRST(LEFTMOST_COLUMN), SECOND(ConstantPositionTableColumn.of(1)), NOT_FOUND(OptionalTableColumn.of(NOCOLUMN));
        ...
        @java.lang.SuppressWarnings("all")
        private Columns(final TableColumn column) {
            this.column = column;
        }
    }
}
